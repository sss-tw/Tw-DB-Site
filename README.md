...
网站本身是纯静态前端，可以部署在 GitHub Pages；数据来自 Turso/libSQL 远端数据库，前端使用只读 token 查询数据。
...
- 支持 GitHub Pages 静态部署，数据库独立托管在 Turso。

## 技术结构

- `index.html`：页面入口。
- `app.js`：路由、Turso 查询、列表页和详情页渲染逻辑。
- `style.css`：站点样式。
- `assets/`：本地化后的页面脚本、样式、图标和图片资源。
- `db-config.js`：Turso URL 与只读 token 配置。

## 数据库

本仓库不保存大型数据库文件。当前线上数据已经导入 Turso，前端通过 `db-config.js` 中的只读 token 访问。
